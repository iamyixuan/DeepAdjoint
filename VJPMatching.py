...
class MLP:
    ...
    def init_network(self, key):
        ...
        return [
            init_layer(m, n, k)
            for m, n, k in zip(self.layers[:-1], self.layers[1:], keys)
        ]
    ...
    def nn_adjoint(self, params, x, v):
...
class Trainer:
    ...
    def loss(self, params, x, y, adj_y, alpha):
        ...
        adj = self.net.nn_adjoint(params, x, v)
    ...
    def step_(self, params, x, y, adj_y, alpha, opt_state):
        ...
        (ls, adj_loss), grads = jax.value_and_grad(self.loss, argnums=0, has_aux=True)(
            params, x, y, adj_y, alpha
        )
    ...
    def step_full_Jacobian(self, params, x, y, adj_y, alpha, opt_state):
        (ls, adj_loss), grads = jax.value_and_grad(
            self.loss_full_Jacobian, argnums=0, has_aux=True
        )(params, x, y, adj_y, alpha)
        updates, opt_state = self.optimizer.update(grads, opt_state)
        params = optax.apply_updates(params, updates)
        return params, ls, opt_state, adj_loss
        ...
        for ep in range(self.num_epochs):
            ...
            for i in range(len(x_train) // self.batch_size):
                ...
                if self.if_full_Jacobian == "True":
                    params, ls, opt_state, adj_ls_batch = self.step_full_Jacobian(
                        params, x_batch, y_batch, adj_batch, running_alpha, opt_state
                    )
                else:
                    params, ls, opt_state, adj_ls_batch = self.step_(
                        params, x_batch, y_batch, adj_batch, running_alpha, opt_state
                    )
            ...
            if self.if_full_Jacobian == "True":
                ...
                ls_val, val_adj_loss = self.loss_full_Jacobian(
                    params, x_val, y_val, adj_val, alpha
                )  # set alpha with the preset value in validation.
            else:
                if ep == 0:
                    print("Training with vector-Jacobian product...")
                ls_val, val_adj_loss = self.loss(
                    params, x_val, y_val, adj_val, alpha
                )  # set alpha with the preset value in validation.
            ...
            print(
                "Epoch: {} training loss {:.4f} validation loss {:.4f}".format(
                    ep, logger["train_loss"][-1], logger["val_loss"][-1]
                )
            )
            print(
                "training r2 {:.4f} validation r2 {:.4f}".format(
                    logger["train_r2"][-1], logger["val_r2"][-1]
                )
            )
            print(
