...
class Plotter:
    ...
    def heat_map(self, x_t, x_p):
        ...
        ax[0].imshow(x_t, cmap='jet', interpolation='nearest', origin='lower',
                    extent=[0,50,0,80], aspect='auto', vmax=x_t.max(), vmin=x_t.min())
        ax[0].set_xlabel('time steps')
        ax[0].set_ylabel('spatial locs')
        ax[0].set_title('true')

        ax[1].imshow(x_p, cmap='jet', interpolation='nearest', origin='lower',
                    extent=[0,50,0,80], aspect='auto', vmax=x_t.max(), vmin=x_t.min())
        ax[1].set_xlabel('time steps')
        ax[1].set_ylabel('spatial locs')
        ax[1].set_title('predicted')

        ax[2].imshow(error, cmap='jet', interpolation='nearest', origin='lower',
                    extent=[0,50,0,80], aspect='auto', vmax=x_t.max(), vmin=x_t.min())
    ...
    def save(self, fig, name):
        fig.savefig('../../data/plots/'+name+'.pdf', format='pdf', bbox_inches='tight')
