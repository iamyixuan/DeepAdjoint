...
class Trainer:
    ...
    def train(self, trainSet, valSet):
        ...
        for epoch in range(self.numEpcohs):
            ...
            print(f"Epoch {epoch+1}: Training Loss: {logger['trainLoss'][-1]:.4e}; Validation Loss: {logger['valLoss'][-1]:.4e}")
...
if __name__=='__main__':
    ...
    trainer = Trainer(net.to(device), 1000, 0.0001, 64)
    trainer.train(trainSet, valSet)
    torch.save(net.state_dict(), './saved/forwardModel.pt')
#-----------------------------------------------------------
    ...
    pred = trainer.pred_time_roll_out(net, initCond, 200)
    ...
    ax[0].set_title('True')
    ax[0].imshow(y.T, origin='lower', vmin=vmin, vmax=vmax, extent=[0, 1, 0, 1], transform=ax[0].transAxes)
    ax[0].set_axis_off()
    ax[1].set_box_aspect(.5)
    ax[1].set_axis_off()
    ax[1].set_title('Predicted')
    im =ax[1].imshow(pred.T, origin='lower', vmin=vmin, vmax=vmax, extent=[0, 1, 0, 1], transform=ax[1].transAxes)
    fig.tight_layout()
