...
print(
    "the jac wrt the parameter R2 is {:.4f}".format(
        r2(test["adj"][:2000][:, -1], pred_adj_p)
    )
)
print(
    "The jac parameter only Mse is {}".format(
        mean_squared_error(test["adj"][:2000][:, -1], pred_adj_p)
    )
)
