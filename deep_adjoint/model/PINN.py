...
class BurgerSupervisor:
    ...
    def train(self, train, val):
        ...
        for i in range(self.epochs):
            ...
            print(
                "Epoch %d, training loss: (%.4f, %.4f), validiation PDE loss %.4f"
                % (i + 1, loss_u.numpy(), loss_f.numpy(), val_f_loss)
            )
...
if __name__ == "__main__":
    ...
    sup = BurgerSupervisor(
        nu=NU, net=net, epochs=281, lr=0.016580695, alpha=0.016580695
    )
    train, val, test = get_data(NT=STEPS, NX=N, X_U=1.0, X_L=-1.0, T_max=1.0, Nu=NU)
    sup.train(train, val)
    plotter(
        test["x"], net(test["x"][:, 0], test["x"][:, 1]).numpy(), "test_tune", N, STEPS
    )
