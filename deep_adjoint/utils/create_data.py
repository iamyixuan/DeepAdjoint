...
def get_time_series(data, hist_len, horizon, var_id_x, var_id_y, vertical_level=0, if_two_d=False):
        ...
        mask1 = mask["mask1"]
        mask2 = mask["mask2"]
        mask = np.logical_or(mask1, mask2)[0:1, vertical_level, :, :, 0:1]
        x_ = []
        y_ = []
        ...
        for i in range(data.shape[0] - hist_len - horizon + 1):
            x = data[i : i + hist_len, vertical_level, ..., var_id_x[0:]]
            y = data[i + hist_len : i + hist_len + horizon, vertical_level, ..., var_id_y[0:]]
...
def make_h5data(data_path, dataset_name, data_f, keys, mode):
    for i, k in tqdm(enumerate(keys)):
        x, y = get_time_series(data_f[k], 1, 1, [8, 11, 14, 15, -1], [8, 11, 14, 15])  # Adjust as per your requirements
        ...
        with h5py.File(file_path, 'a') as new_f:
            if i == 0:  # First dataset, create the file and datasets
                new_f.create_dataset('x', shape=(0,) + x.shape[1:], dtype=x.dtype)
                new_f.create_dataset('y', shape=(0,) + y.shape[1:], dtype=y.dtype)
...
if __name__ == "__main__":
    create_datasets('soma_deep_ensemble')
