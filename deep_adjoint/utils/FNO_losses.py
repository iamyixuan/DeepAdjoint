...
class H1Loss(object):
    ...
    def abs(self, x, y, h=None):
        ...
        const = math.prod(h)
        diff = (
            const * torch.norm(dict_x[0] - dict_y[0], p=2, dim=-1, keepdim=False) ** 2
        )

        for j in range(1, self.d + 1):
            diff += (
                const
                * torch.norm(dict_x[j] - dict_y[j], p=2, dim=-1, keepdim=False) ** 2
            )
...
class MSE_ACCLoss(torch.nn.Module):
    ...
    def forward(self, true, pred, mask=None):
        ...
        std = torch.sqrt(
            torch.mean(PredAnomaly**2, dim=(0, 2, 3, 4))
            * torch.mean(TrueAnomaly**2, dim=(0, 2, 3, 4))
        )
