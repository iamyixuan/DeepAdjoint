...
def process_single_month(forward, args, lock):
    results_year = []
    ...
    with lock:
        with h5py.File(
            f"{args.SAVE_PATH}/data-{args.PAR_NAME}.hdf5", "a"
        ) as f:
            f.create_dataset(f"forward_{forward}", data=full_result, dtype="f")
...
if __name__ == "__main__":
    ...
    pool.starmap(process_single_month, [(i, args, lock) for i in range(100)])
