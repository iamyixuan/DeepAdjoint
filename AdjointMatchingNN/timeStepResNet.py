...
class Data(Dataset):
    ...
    def __getitem__(self, idx):
        return torch.from_numpy(self.data['x'][idx]).float(), (torch.from_numpy(self.data['y'][idx]).float(), torch.from_numpy(self.data['adj'][idx]).float())
...
class Trainer:
    ...
    def _train(self, train, val, alpha):
        ...
        for epoch in range(self.num_epochs):
            ...
            self.logger['train_loss'].append(np.mean(running_losses.reshape(-1, 1), axis=1))
            self.logger['val_loss'].append(np.array([val_loss.detach().numpy(), aux_val[0].detach().numpy(), aux_val[1]]))
            
            trainTotLoss, trainMSELoss, trainAdjLoss = self.logger['train_loss'][-1]
            valTotLoss, valMSELoss, valAdjLoss = self.logger['val_loss'][-1]
            print(f'Epoch {epoch+1}; Training loss: [{trainTotLoss:.3f}, {trainMSELoss:.3f}, {trainAdjLoss:.3f}]; Validation loss: [{valTotLoss:.3f}, {valMSELoss:.3f}, {valAdjLoss:.3f}]')
...
if __name__ == '__main__':
    ...
    trainer = Trainer(net=net, num_epochs=1000, batch_size=64, learning_rate=0.001, optimizer=torch.optim.Adam)
